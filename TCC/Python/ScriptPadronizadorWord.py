...
from SciprtPadronizadorAuxiliar import ler_abreviacoes, padronizar_nome, formatar_tabela, adicionar_estilo_titulo, adicionar_quebra_pagina, verificar_nomes
...
# Função para adicionar aviso no final do documento Word com a mesma formatação do título "Linha"
def adicionar_aviso_no_word(doc, texto_aviso):
    # Adicionar uma quebra de página antes do aviso
    doc.add_page_break()
    
    # Adicionar o aviso com a mesma formatação de título "Linha"
    aviso_titulo = doc.add_heading("Aviso:", level=2)  # Defina o nível apropriado se "Linha" usa outro nível
    aviso_titulo.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
    
    paragrafo_aviso = doc.add_paragraph(texto_aviso)
    paragrafo_aviso.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
...
def aplicar_padronizacao(caminho_abreviacoes, caminho_planilha, caminho_saida):
    ...
    # Verificação de nomes e obtenção de mensagem de aviso
    lista_nomes = df['COLUMN BUSINESS NAME'].tolist() if 'COLUMN BUSINESS NAME' in df.columns else []
    texto_aviso = verificar_nomes(caminho_abreviacoes, lista_nomes)

    # Adicionar aviso no final do documento
    adicionar_aviso_no_word(doc, texto_aviso)
